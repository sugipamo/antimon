    ...
    def format_security_issue(self, message: str) -> str:
        ...
        for line in lines:
            ...
            if not line:
                ...
                formatted_lines.append(f"      {self.warning('Pattern matched:')} {self.highlight(line.split(':', 1)[1].strip())}")
            elif line.startswith('Why:'):
                formatted_lines.append(f"      {self.info('Why:')} {line.split(':', 1)[1].strip()}")
            elif line.startswith('Suggestion:'):
                formatted_lines.append(f"      {self.success('Suggestion:')} {line.split(':', 1)[1].strip()}")
            else:
                # Main error message

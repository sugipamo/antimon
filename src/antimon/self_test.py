    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if test_case["should_detect"]:
            if has_issues:
                ...
                if pattern_found:
                    print(
                        f"  ✅ PASS: Correctly detected {test_case['expected_pattern']}"
                    )
                    if verbose:
                    ...
                    print(
                        f"  ❌ FAIL: Detected issues but not the expected pattern '{test_case['expected_pattern']}'"
                    )
                    print(f"  Messages: {messages}")
                    failed += 1
            else:
                print(
                    f"  ❌ FAIL: Should have detected {test_case['expected_pattern']} but didn't"
                )
                failed += 1
        else:
            if not has_issues:
